        	...
        	<table class="table apptable table-condensed medShadow" style="background:white;" id="dtable2">
            	<thead>
            	    <tr>
            	        ...
            	        <th>Status</th>
            	        <th>System Type</th>
            	        <th style='width:4%;'>MAJ#360</th>
            	        <th style='width:4%;'>DEF 1</th>
            	        <th style='width:4%;'>DEF 2</th>
            	        <th style='width:4%;'>ATT#360</th>
                            ...
                            <td>{{$count}}</td>
                              ...
                              <td id='status-{{$val->id}}'>
                                    ...
                                    if($val->cancel_date!='0000-00-00'){
                                          $refund = date('M-d',strtotime('+15 Days',strtotime($val->cancel_date)));
                              ...
                              <?php if($val->typeofsale=="2maj") {$systemType="2 Majestics";} 
                              ...
                              <?php 
                                    ...
                                    $val->maj = Sale::inventorySKU($val->maj);
                                    $val->twomaj = Sale::inventorySKU($val->twomaj);
                                    $val->threemaj = Sale::inventorySKU($val->threemaj);
                                    $val->att = Sale::inventorySKU($val->att);
                                    $val->defone = Sale::inventorySKU($val->defone);
                                    $val->deftwo = Sale::inventorySKU($val->deftwo);
                                    $val->defthree = Sale::inventorySKU($val->defthree);
                                    $val->deffour = Sale::inventorySKU($val->deffour);
                                    $val->deffive = Sale::inventorySKU($val->deffive);
                                    $val->maj_old = Sale::inventorySKU($val->maj_old);
                                    $val->twomaj_old = Sale::inventorySKU($val->twomaj_old);
                                    $val->threemaj_old = Sale::inventorySKU($val->threemaj_old);
                                    $val->att_old = Sale::inventorySKU($val->att_old);
                                    $val->defone_old = Sale::inventorySKU($val->defone_old);
                                    $val->deftwo_old = Sale::inventorySKU($val->deftwo_old);
                                    $val->defthree_old = Sale::inventorySKU($val->defthree_old);
                                    $val->deffour_old = Sale::inventorySKU($val->deffour_old);
                                    $val->deffive_old = Sale::inventorySKU($val->deffive_old);
                              ;?>
                              @if(($val->status=="CANCELLED")||($val->status=="TURNDOWN"))
                              @if($val->picked_up==1)
                              <td class='machines' id='maj-{{$val->id}}'>
                                    @if($val->maj_old!=false)
                                    <span class='label' id='maj-span-{{$val->id}}'>{{$val->maj_old}}</span>
                                    @endif
                                    @if($val->twomaj_old!=false)
                                    <span class='label' id='twomaj-span-{{$val->id}}'>{{$val->twomaj_old}}</span>
                                    @endif
                                     @if($val->threemaj_old!=false)
                                    <span class='label' id='threemaj-span-{{$val->id}}'>{{$val->threemaj_old}}</span>
                                    @endif
                              </td>
                              <td class='machines' id='def-{{$val->id}}' >
                                    <center>
                                          @if($val->defone_old!=false)
                                          <span class='label' id='defone-span-{{$val->id}}'>{{$val->defone_old}}</span>
                                          @endif
                                    </center>
                              </td>
                              <td class='machines' id='def2-{{$val->id}}'>
                                    <center>
                                    @if($val->deftwo_old!=false)
                                    <span class='label' id='deftwo-span-{{$val->id}}'>{{$val->deftwo_old}}</span>
                                    @endif
                                    @if($val->defthree_old!=false)
                                    <span class='label' id='defthree-span-{{$val->id}}'>{{$val->defthree_old}}</span>
                                    @endif
                                    @if($val->deffour_old!=false)
                                    <span class='label' id='deffour-span-{{$val->id}}'>{{$val->deffour_old}}</span>
                                    @endif
                                    @if($val->deffive_old!=false)
                                    <span class='label' id='deffive-span-{{$val->id}}'>{{$val->deffour_old}}</span>
                                    @endif
                                    </center>
                              </td>
                              <td class='machines' id='att-{{$val->id}}' >
                                    @if($val->att_old!=false)
                                    <span class='label' id='att-span-{{$val->id}}'>{{$val->att_old}}</span>
                                    @endif
                              </td>
                              @else
                              ...
                              <td class='machines' id='maj-{{$val->id}}'>
                                    @if($val->maj!=false)
                                    <span class='label label-important' id='maj-span-{{$val->id}}'>{{$val->maj}}</span>
                                    @endif
                                     @if($val->twomaj!=false)
                                    <span class='label label-important' id='twomaj-span-{{$val->id}}'>{{$val->twomaj}}</span>
                                    @endif
                                     @if($val->threemaj!=false)
                                    <span class='label label-important' id='threemaj-span-{{$val->id}}'>{{$val->threemaj}}</span>
                                    @endif
                              </td>
                              <td class='machines' id='def-{{$val->id}}'>
                                    <center>
                                    @if($val->defone!=false)
                                    <span class='label label-important' id='defone-span-{{$val->id}}'>{{$val->defone}}</span>
                                    @endif
                              ...
                              <td class='machines' id='def2-{{$val->id}}'>
                                    <center>
                                    @if($val->deftwo!=false)
                                    <span class='label label-important' id='deftwo-span-{{$val->id}}'>{{$val->deftwo}}</span>@endif
                                    @if($val->defthree!=false)
                                    <span class='label label-important' id='defthree-span-{{$val->id}}'>{{$val->defthree}}</span>@endif
                                    @if($val->deffour!=false)
                                    <span class='label label-important' id='deffour-span-{{$val->id}}'>{{$val->deffour}}</span>@endif
                                     @if($val->deffive!=false)
                                    <span class='label label-important' id='deffive-span-{{$val->id}}'>{{$val->deffive}}</span>@endif
                                    </center>
                              </td>
                              
                              <td class='machines' id='att-{{$val->id}}'>
                                    @if($val->att!=false)
                                    <span class='label label-important' id='att-span-{{$val->id}}'>{{$val->att}}</span>
                                    @endif
                              </td>
                              @endif
                              @else

                              @if(($val->typeofsale=="majestic")||($val->typeofsale=="system")||($val->typeofsale=="supersystem")||($val->typeofsale=="megasystem")||($val->typeofsale=="novasystem")||($val->typeofsale=="supernova")||($val->typeofsale=="2system")||($val->typeofsale=="2maj")||($val->typeofsale=="3maj"))
                              <td class='machines' id='maj-{{$val->id}}'><center>  
                                    @if(!empty($val->maj))
                                    <span class="removeMachine bordbut label label-info special" id="maj-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="maj"  data-type="machines" >{{$val->maj}}</span>
                                    @else 
                                    <span class="revealDetails bordbut label label-warning blacktext" id="maj-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="maj"  data-type="machines">Pick SKU</span>
                                    @endif

                                    @if($val->typeofsale=="2maj" || $val->typeofsale=="3maj" || $val->typeofsale=="2system")
                                    @if(!empty($val->twomaj))
                                    <span class="removeMachine bordbut label label-info special" id="twomaj-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="twomaj"  data-type="machines" >{{$val->twomaj}}</span>
                                    @else 
                                    <span class="revealDetails bordbut label label-warning blacktext" id="twomaj-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="twomaj"  data-type="machines">Pick SKU</span>
                                    @endif
                                    @endif

                                    @if($val->typeofsale=="3maj")
                                    @if(!empty($val->threemaj))
                                    <span class="removeMachine bordbut label label-info special" id="threemaj-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="threemaj"  data-type="machines" >{{$val->threemaj}}</span>
                                    @else 
                                    <span class="revealDetails bordbut label label-warning blacktext" id="threemaj-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine=three"maj"  data-type="machines">Pick SKU</span>
                                    @endif
                                    @endif
                              ...
                              @else 
                              <td id='maj-{{$val->id}}' class='nomachine machines'></td>
                              @endif


                              <td class='machines' id="def-{{$val->id}}" >  
                              <center>  
                              @if(($val->typeofsale=="defender")||($val->typeofsale=="2defenders")||($val->typeofsale=="3defenders")||($val->typeofsale=="system")||($val->typeofsale=="supersystem")||($val->typeofsale=="megasystem")||($val->typeofsale=="novasystem")||($val->typeofsale=="supernova")||($val->typeofsale=="2system"))
                              
                                    @if(!empty($val->defone))
                                    <span class="removeMachine bordbut label label-info special" id="defone-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="defone"  data-type="machines" >{{$val->defone}}</span>
                                    @else 
                                    <span class="revealDetails bordbut label label-warning blacktext" id="defone-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="defone"  data-type="machines" >Pick SKU</span>
                                    @endif
                              @endif
                          </center>
                          <td>

                              @if(($val->typeofsale=="supersystem")||($val->typeofsale=="megasystem")||($val->typeofsale=="novasystem")||($val->typeofsale=="supernova")||($val->typeofsale=="2system")||($val->typeofsale=="2defenders")||($val->typeofsale=="3defenders"))
                                    @if(!empty($val->deftwo))
                                    <span class="removeMachine bordbut label label-info special" id="deftwo-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="deftwo"  data-type="machines" >{{$val->deftwo}}</span>
                                    @else 
                                    <span class="revealDetails bordbut label label-warning blacktext" id="deftwo-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="deftwo" data-type="machines">Pick SKU</span>
                                    @endif
                              @else
                              @endif

                              @if(($val->typeofsale=="megasystem")||($val->typeofsale=="novasystem")||($val->typeofsale=="supernova")||($val->typeofsale=="3defenders"))
                                    @if(!empty($val->defthree))
                                    <span class="removeMachine bordbut label label-info special" id="defthree-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="defthree"  data-type="machines" >{{$val->defthree}}</span>
                                    @else 
                                          <span class="revealDetails bordbut label label-warning blacktext" id="defthree-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="defthree" data-type="machines">Pick SKU</span>
                                    @endif
                              ...
                              @if(($val->typeofsale=="novasystem")||($val->typeofsale=="supernova")||($val->typeofsale=="4defenders"))
                                    @if(!empty($val->deffour))
                                    <span class="removeMachine bordbut label label-info special" id="deffour-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="deffour"  data-type="machines" >{{$val->deffour}}</span>
                                    @else 
                                          <span class="revealDetails bordbut label label-warning blacktext" id="deffour-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="deffour" data-type="machines">Pick SKU</span>
                                    @endif
                              @else 
                              @endif
                              @if($val->typeofsale=="supernova")
                                    @if(!empty($val->deffive))
                                    <span class="removeMachine bordbut label label-info special" id="deffive-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="deffive"  data-type="machines" >{{$val->deffive}}</span>
                                    @else 
                                          <span class="revealDetails bordbut label label-warning blacktext" id="deffive-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="deffive" data-type="machines">Pick SKU</span>
                                    @endif
                              @else 
                              @endif
                              </center>
                              ...
                              @if(($val->typeofsale=="system")||($val->typeofsale=="2system")||($val->typeofsale=="supersystem")||($val->typeofsale=="supernova")||($val->typeofsale=="megasystem")||($val->typeofsale=="novasystem")||($val->typeofsale=="majestic")||($val->typeofsale=="2maj")||($val->typeofsale=="3maj"))
                              <td  class='machines' id='att-{{$val->id}}' >  <center>  
                                    @if(!empty($val->att))
                                    <span class="removeMachine bordbut label label-info special" id="att-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="att"  data-type="machines" >{{$val->att}}</span>
                                    @else 
                                          <span class="revealDetails bordbut label label-warning blacktext" id="att-span-{{$sale->id}}" data-id="{{$sale->id}}" data-machine="att" data-type="machines">Pick SKU</span>
                                    @endif
                                    </center>
                                    <!--TODO 3 attachments-->
                              </td>
                              @else 
                              <td id='deftwo-{{$val->id}}' class='nomachine machines'></td>
                              @endif
                              @endif
                              <td> 
...
$('#dtable2').dataTable({
	...
	$(document).on('click','.removeMachine',function(){
        ...
        if(t){
            $.getJSON('{{URL::tO("sales/removeitem/")}}',{machine:machine,sale:id},function(data){
                if(data=="success"){
                    toastr.success("Removed this item from sale","Returned to Stock");
                    thebut.removeClass().addClass('revealDetails bordbut label label-warning blacktext').html("Pick SKU");
                } else if(data=="failed") {
	...
	$('.statusedit').editable('{{URL::to("sales/edit")}}',{
		...
		data : '{"":"0.00 %",19.95:"19.95 %",29.9:"29.90 %"}',
		type:'select',
		submit:'OK',
    		indicator : 'Saving...',
    		placeholder:".......",
	...
	$('.systemedit').editable('{{URL::to("sales/edit")}}',{
		data : '{"defender":"Defender","2defenders":"2 Defenders","3defenders":"3 Defenders","majestic":"Majestic","2maj":"2 Majestics","3maj":"3 Majestics","system":"System","supersystem":"Super System","megasystem":"Mega System","novasystem":"Nova System","supernova":"Super Nova System","2system":"2 Systems"}',
		type: 'select',
		submit:'OK',
		indicator: 'saving',
		callback: function(value, settings){
