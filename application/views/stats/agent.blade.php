...
<div id="main" role="main" class="container-fluid">
    <div class="contained">
        ...
        <!-- MAIN CONTENT -->
        <div id="page-content" >
            <h1 id="page-header">Booker Stats</h1>   
            <div id="element"></div>
                ...
                <section id="widget-grid" class="">
                                    ...
                                    data-widget-fullscreenbutton="false" >
                                        ...
                                        <tbody class='stats doortable'>
                                           @foreach($totalstats as $val)
                                            ...
                                            <td><center><span class='closepercent doorclose'>@if($val->appdoor!=0){{number_format(($val->appdoor/($val->appdoor+$val->nidoor)*100),2,'.','')}}@endif</span></center></td>
                                            <td><center><span class="label label-success white doorapp">{{$val->appdoor}}</span></center></td>
                                            <td><center><span class="label label-important white doorni">{{$val->nidoor}}</span></center></td>
                                            <td><center><span class='doornh'>{{$val->nhdoor}}</span></center></td>
                                            <td><center><span class='nqdoor'>{{$val->nqdoor}}</span></center></td>
                                            <td><center><span class='dncdoor'>{{$val->dncdoor}}</span></center></td>
                                            <td><center><span class='wrongdoor'>{{$val->wrongdoor}}</span></center></td>
                                            <td><center><span class='recalldoor'>{{$val->recalldoor}}</span></center></td>
                                            <td><center><span class='totdoor'>{{$val->totdoor}}</span></center></td>
                                            @endif
                                            </tr>
                                            @endforeach
                                            </tbody>
                                        ...
                                        <table class="table table-bordered table-condensed paper stattable " style="border:1px solid #1f1f1f;display:none;" >
                                        <thead style="color:#000!important">
                                            <th>Booker</th>
                                            <th>CLOSE %</th>
                                            <th>APP</th>
                                        ...
                                        <tbody class='stats papertable'>
                                           @foreach($totalstats as $val)
                                            ...
                                            <tr><td>{{strtoupper($u->firstname)}} {{strtoupper($u->lastname)}}</td>
                                            <td><center><span class='closepercent'>@if($val->apppaper!=0) {{number_format(($val->apppaper/($val->apppaper+$val->nipaper)*100),2,'.','')}}@endif</span></center></td>
                                            <td><center><span class="label label-success white">{{$val->apppaper}}</span></center></td>
                                            <td><center><span class="label label-important white">{{$val->nipaper}}</span></center></td>
                                            <td><center>{{$val->nhpaper}}</center></td>
                                            <td><center>{{$val->nqpaper}}</center></td>
                                            <td><center>{{$val->dncpaper}}</center></td>
                                            <td><center>{{$val->wrongpaper}}</center></td>
                                            <td><center>{{$val->recallpaper}}</center></td>
                                            <td><center>{{$val->totpaper}}</center></td>
                                            @endif
                                            </tr>
                                            @endforeach
                                            </tbody>
                                        ...
                                        <table class="table table-bordered table-condensed other stattable " style="border:1px solid #1f1f1f;display:none;" >
                                        <thead style="color:#000!important">
                                            <th>Booker</th>
                                            <th>CLOSE %</th>
                                            <th>APP</th>
                                        ...
                                        <tbody class='stats othertable'>
                                           @foreach($totalstats as $val)
                                            ...
                                            <td>
                                                <center>
                                                <span class='closepercent'>
                                                @if($val->appother!=0) {{number_format(($val->appother/($val->appother+$val->niother)*100),2,'.','')}}@endif
                                                </span>
                                                </center>
                                            </td>
                                            <td><center><span class="label label-success white">{{$val->appother}}</span></center></td>
                                            <td><center><span class="label label-important white">{{$val->niother}}</span></center></td>
                                            <td><center>{{$val->nhother}}</center></td>
                                            <td><center>{{$val->nqother}}</center></td>
                                            <td><center>{{$val->dncother}}</center></td>
                                            <td><center>{{$val->wrongother}}</center></td>
                                            <td><center>{{$val->recallother}}</center></td>
                                            <td><center>{{$val->totother}}</center></td>
                                            @endif
                                            </tr>
                                            @endforeach
                                            </tbody>
                    ...
                    <div class="row-fluid" id="availableLeads">
                            ...
                            <article class="span12">
                                    ...
                                    data-widget-fullscreenbutton="false" >
                                        <header>
                                            <h2>TOTAL CALLS MADE - {{$totalstats[0]->total}}</h2>  
        ...
        <aside class="right">
                                            ...
                                            @if($u->user_type=="agent")
                                                <div class="topsalesbox">
                                                            ...
                                                            <h4 class="shadow">{{$u->firstname}} {{$u->lastname}}</h4>
                                                            <span class="label label-info">{{$val->nidoor+$val->nipaper+$val->niother}} Contacts</span>
                                                            <span class="label label-success">{{$val->appdoor+$val->apppaper+$val->appother}} Demos Booked</span>
                                                            
                                                            <h5 style="color:#000;">Close Percentage</h5>
                                                            <div class="progress progress-success" style="margin-top:10px;">
                                                                <div class="bar filled-text" data-percentage="{{number_format(($val->appdoor/($val->appdoor+$val->nidoor)*100),2,'.','')}}">
                                                                    >{{number_format(($val->appdoor/($val->appdoor+$val->nidoor)*100),2,'.','')}}%
                                                                </div>
                                                            </div>
