	...
	**/
	...
	function isMobile() {
	    ...
	    if(!ismobile){
			...
			$('#filterdate').datepicker('format', 'yyyy/mm/dd').on('changeDate', function(ev){
				var newDate = new Date(ev.date);
				newDate.setDate(newDate.getDate() + 1);
				var timestr = $.datepicker.formatDate('yy-mm-dd', newDate);
				window.location.href = "?date="+timestr;
    });
...
*/
	...
	function setup_chatbox_demo() {
		...
		$('.getmessages').on('click', function(){
			...
			var id = $(this).data('id');
			var url = "chat/getmsgs/"+id;
			
			$("#chatview-"+id).load(url);
   			var refreshId = setInterval(function() {
      		$("#chatview-"+id).load("chat/getmsgs/"+id);
   			}, 5000);
   			$.ajaxSetup({ cache: false });
		
		});

		$('#chattoagent').change(function(){
			var id = $(this).val();
			window.location.href = "/chat/assigntab/"+id;
		});

		$('.chatclose').on('click', function(){
			var id = $(this).data('id');
			$.post("chat/deletetab", { tab_id: id}).done(function(data) {
				$('#chat-'+id).hide();
			});
		...
		$('.sendmessage').click(function() {
			...
			var msg_input = $('#chatboxtext-'+id).val();
			if (msg_input.length) {
				$.post("chat/newmsg", { receiver_id: id, message: msg_input })
				.done(function(data) {
				$('#chatboxtext-'+id).val('');
				var url = "chat/getmsgs/"+id;
				$("#chatview-"+id).load(url);
	...
	function setup_bootbox_demo() {
		if ($('.bootbox-js').length){
			
		
			$('.bootbox-js .bootbox-confirm-js').click(function(e) {
				e.preventDefault();
