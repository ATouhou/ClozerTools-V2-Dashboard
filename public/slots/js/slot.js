		...
		var c = $('.credits').html();
			...
			addCredit = function(incrementCredits) {
				...
				$('#slotCredits')
					...
					.animate({
						...
						complete: function() {
							$.getJSON('../../users/credits?type=inc&amount='+incrementCredits, function(data){
								$('.spins').html(data.spins);
			...
			spin = function() {
				this.blur();
					if(spins==0){
						spinning = true;
						alert('No More Spins Left, please book more demos!!');
						return false;
					}

					if(credits==0){
						spinning = true;
						alert('No More credits Left, please book more demos!!');
						return false;
					}
					...
					credits --;
					spins--;
					$.getJSON('../../users/credits?type=dec',function(data){
						$('.spins').html(data.spins);
						$('.credits').html(data.credits);
					});
					$('#slotCredits').html(credits);
